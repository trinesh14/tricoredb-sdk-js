...
test('quick start', { skip }, async () => {
  ...
  const after = await db.query('SELECT name FROM users WHERE id = ?', [1]);
  assert.equal(after.rows[0][0], 'Grace');
});

test('transactions', { skip }, async () => {
...
test('connection pool', { skip }, async () => {
...
test('errors carry a stable code', { skip }, async () => {
